...
function touchpro_preprocess_html(&$variables) {
    $node = node_load(arg(1));
    $results = taxonomy_node_get_terms($node);
    if(is_array($results)) {
        foreach ($results as $item) {
           $variables['classes_array'][] = "taxonomy-".str_replace(array('æ','ø','å'),array('ae','oe','aa'),strtolower(drupal_clean_css_identifier($item->name))); 
        }
    }
}
